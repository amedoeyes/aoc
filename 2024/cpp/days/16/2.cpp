...
auto main(int /*argc*/, char** argv) -> int {
	...
	auto best_paths = std::unordered_set<vector<2, int64_t>, vector_hash>();
	auto score = std::unordered_map<std::pair<vector<2, int64_t>, int64_t>, int64_t, pair_hash>();
	auto pqueue = std::priority_queue<
		std::tuple<vector<2, int64_t>, int64_t, int64_t, std::vector<vector<2, int64_t>>>,
		std::vector<std::tuple<vector<2, int64_t>, int64_t, int64_t, std::vector<vector<2, int64_t>>>>,
		cmp>();
	score[{start, 1}] = 0;
	pqueue.emplace(start, 0, 1, std::vector<vector<2, int64_t>>());
	auto min = std::numeric_limits<int64_t>::max();
	while (!pqueue.empty()) {
		const auto [n, s, d, p] = pqueue.top();
		pqueue.pop();
		if (n == end && (min == std::numeric_limits<int64_t>::max())) min = s;
		if (s > min) continue;
		if (n == end && s == min) {
			for (const auto& n : p) best_paths.emplace(n);
			continue;
		}
